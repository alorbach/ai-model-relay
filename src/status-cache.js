...
function createStatusCache(context, onUpdate = () => {}) {
	...
	function refresh() {
		...
		refreshing = Promise.resolve(context.backends.refresh ? context.backends.refresh() : context.backends.capabilities()).then(() => {
			...
			status = { ...codexStatus, bridge: status.bridge, asr: context.codex.asrStatus ? context.codex.asrStatus({ refresh: true }) : {}, music_analysis: context.musicAnalysis && context.musicAnalysis.capabilities ? context.musicAnalysis.capabilities() : {}, jobs: context.jobManager.snapshot(), checking: false, last_checked: now, refresh: refreshState };
