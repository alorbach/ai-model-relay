...
function createStatusCache(context, onUpdate = () => {}) {
	...
	let status = { success: false, message: 'Checking local model providers in background.', details: { checking: true }, bridge: {}, asr: context.codex.asrStatus ? context.codex.asrStatus() : {}, music_analysis: context.musicAnalysis && context.musicAnalysis.capabilities ? context.musicAnalysis.capabilities() : {}, jobs: context.jobManager.snapshot(), checking: true, last_checked: null, refresh: refreshState };
	let capabilities = { success: true, codex: initialCodexCapabilities.details || { checking: true }, asr: context.codex.asrStatus ? context.codex.asrStatus() : {}, music_analysis: context.musicAnalysis && context.musicAnalysis.capabilities ? context.musicAnalysis.capabilities() : {}, features: initialCodexCapabilities.features || {}, backends: context.backends.capabilities(), video: context.video.capabilities ? context.video.capabilities() : { enabled: false }, media_analysis: context.mediaAnalysis.capabilities ? context.mediaAnalysis.capabilities() : { enabled: false }, checking: true, last_checked: null, refresh: refreshState };
	function sync() {
		capabilities = {
			...capabilities,
			asr: context.codex.asrStatus ? context.codex.asrStatus() : {},
			music_analysis: context.musicAnalysis && context.musicAnalysis.capabilities ? context.musicAnalysis.capabilities() : {},
	...
	function refresh() {
		...
		refreshing = Promise.resolve(context.backends.refresh ? context.backends.refresh() : context.backends.capabilities()).then(() => {
			...
			status = { ...codexStatus, bridge: status.bridge, asr: context.codex.asrStatus ? context.codex.asrStatus() : {}, music_analysis: context.musicAnalysis && context.musicAnalysis.capabilities ? context.musicAnalysis.capabilities() : {}, jobs: context.jobManager.snapshot(), checking: false, last_checked: now, refresh: refreshState };
			capabilities = { ...capabilities, codex: codexCapabilities.details || {}, asr: context.codex.asrStatus ? context.codex.asrStatus() : {}, music_analysis: context.musicAnalysis && context.musicAnalysis.capabilities ? context.musicAnalysis.capabilities() : {}, features: codexCapabilities.features || {}, backends: context.backends.capabilities(), video: context.video.capabilities ? context.video.capabilities() : { enabled: false }, media_analysis: context.mediaAnalysis.capabilities ? context.mediaAnalysis.capabilities() : { enabled: false }, checking: false, last_checked: now, refresh: refreshState };
			onUpdate(status, capabilities);
